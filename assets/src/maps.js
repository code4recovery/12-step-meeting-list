...
function formatLink(url, text, exclude) {
  ...
  if (location.search) {
    ...
    for (var i = 0; i < query_pairs.length; i++) {
      ...
      if (query_parts[0] != exclude)
        new_query_pairs[new_query_pairs.length] =
          query_parts[0] + "=" + query_parts[1];
    }
...
function setMapMarker(title, position, content) {
  ...
  if (mapMode == "google") {
        ...
        (function(marker) {
          return function() {
            infowindow.setContent(
              '<div class="tsml_infowindow">' + content + "</div>"
            );
    ...
    el.style.backgroundImage =
      "url(data:image/svg+xml;base64," + locationIcon + ")";
    el.style.width = "26px";
...
function setMapMarkers(locations, searchLocation) {
  ...
  if (searchLocation) {
    if (tsml.debug)
      console.log("setMapMarker() searchLocation", searchLocation);
  ...
  for (var i = 0; i < location_array.length; i++) {
      ...
      if (location.meetings && location.meetings.length) {
        ...
        for (var j = 0; j < location.meetings.length; j++) {
          ...
          if (current_day != meeting.day) {
            ...
            if (typeof tsml.days[current_day] !== "undefined")
              content += "<h5>" + tsml.days[current_day] + "</h5>";
            content += "<dl>";
          }
          content +=
            "<dt>" +
    ...
    if (typeof marker == "object" && marker) {
      if (mapMode == "google") {
        ...
        if (!bounds.south || position.lat < bounds.south)
          bounds.south = position.lat;
        if (!bounds.east || position.lng > bounds.east)
...
function setSearchMarker(data) {
  ...
  if (mapMode == "google") {
    ...
    el.className = "marker";
    el.style.backgroundImage =
      "url(data:image/svg+xml;base64," + searchIcon + ")";
    el.style.width = "26px";
    el.style.height = "38.4px";
