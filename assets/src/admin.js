jQuery(function ($) {
	...
	function runImport() {
		...
		$.getJSON(tsml.ajaxurl + '?action=tsml_import', function (data) {
			...
			var $errors = $('#tsml_import_errors');

			$progress.toggleClass('has_errors', Boolean(data.errors.length));
			...
			if (!data.counts) {
				console.warn('We likely ran into a concurrent earlier import. Stopping.', data.errors);
