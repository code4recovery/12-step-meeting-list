		...
		$.post(tsml.ajaxurl, controls, function(response){
			...
			if (typeof response != 'object' || response == null) {
				...
				$.each(response, function(index, obj){
					...
					let isMeetingTempClosedAndOnline = (obj.types.indexOf('ONL') !== -1) && (obj.types.indexOf('TC') !== -1)
					console.log(obj.name, isMeetingTempClosedAndOnline)
					//add type 'flags'
					if (typeof tsml.flags == 'object') {
						for (var i = 0; i < tsml.flags.length; i++) {

							if ((obj.types.indexOf(tsml.flags[i]) !== -1) && (isMeetingTempClosedAndOnline && tsml.flags[i] !== 'TC')) {
