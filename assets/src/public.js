...
jQuery(function ($) {
	...
	$('#meeting #feedback').validate({
		...
		submitHandler: function (form) {
			...
			$.post(tsml.ajaxurl, $form.serialize(), function (data) {
				...
				$feedback
					.find('.list-group')
					.html('<li class="list-group-item has-error">' + tsml.strings.email_not_sent + '</li>');
			});
	...
	//dynamically load conference info
	if ($('#meeting a.conference-info').size()) {
		$.get(tsml.ajaxurl, {
			nonce: tsml.nonce,
			action: 'conference_info',
			meeting_id: tsml_map.meeting_id,
		}).then(function (data) {
			$('a#conference_url').attr('href', data.conference_url);
			$('a#conference_phone').attr('href', 'tel:' + data.conference_phone.replace(/[^0-9,#+]/g, ''));
		});
	}
		...
		.on('click', '.dropdown-menu a', function (e) {
			...
			if (param == 'type') {
				if ($('#type li.active a[data-id]').size()) {
					if (tsml.debug)
						console.log('dropdown click ' + $('#type li.active a[data-id]').size() + ' types selected');
					var types = [];
	...
	function getMeetings(controls) {
		...
		for (var property in controls) {
			if (
				controls[property] === null ||
				controls[property] === undefined ||
				!controls[property].toString().length
			) {
				...
				if (typeof response != 'object' || response == null) {
					...
					$.each(response, function (index, obj) {
						...
						if (typeof tsml.flags == 'object') {
							...
							var meetingIsOnlineAndTC =
								obj.types.indexOf('TC') !== -1 && obj.types.indexOf('ONL') !== -1;
						...
						for (var i = 0; i < tsml.columns.length; i++) {
							switch (tsml.columns[i]) {
								case 'time':
									var sort_time =
										(typeof obj.day === 'undefined' ? 7 : obj.day) +
										'-' +
										(obj.time == '00:00' ? '23:59' : obj.time);
									row +=
										'<td class="time" data-sort="' +
										sort_time +
										'-' +
										sanitizeTitle(obj.location) +
										'"><span>' +
								...
								case 'address':
									row +=
										...
										sanitizeTitle(obj.formatted_address) +
										'-' +
										sort_time +
										'">' +
								...
								case 'region':
									row +=
										...
										sanitizeTitle(obj.sub_region || obj.region || '') +
										'-' +
										sort_time +
										'">' +
								...
								case 'district':
									row +=
										...
										sanitizeTitle(obj.sub_district || obj.district || '') +
										'-' +
										sort_time +
										'">' +
