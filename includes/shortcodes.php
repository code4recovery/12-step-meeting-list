...
if (!function_exists('tsml_react')) {
	function tsml_react() {
		...
		$host = (isSecure() ? 'https' : 'http') . '://react.' . (tsml_string_ends(get_site_url(), '.test') ? 'test' : 'meetingguide.org');
		...
		$arr = apply_filters( 'tsml_react_config_array', $defaults );

		wp_localize_script('tsml_react', 'tsml_react_config', $arr);
