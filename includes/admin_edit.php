...
function tsml_admin_init() {
	...
	add_meta_box('info', __('Meeting Information', 'tsml'), 'tsml_meeting_box', TSML_TYPE_MEETINGS, 'normal', 'low');
	...
	function tsml_meeting_box() {
		...
		<div class="meta_form_row">
			<label for="day"><?php _e('Day', 'tsml')?></label>
			<select name="day" id="day">
				...
				<option value=""<?php if (!strlen(@$meeting_custom['day'][0])) {?> selected<?php }?>><?php _e('Appointment', 'tsml')?></option>
			</select>
		</div>
		<div class="meta_form_row">
			<label for="time"><?php _e('Time', 'tsml')?></label>
			<input type="time" name="time" id="time" value="<?php echo @$meeting_custom['time'][0]?>"<?php if (!strlen(@$meeting_custom['day'][0])) {?> disabled<?php }?>>
		</div>
		<div class="meta_form_row">
			<label for="tags"><?php _e('Types', 'tsml')?></label>
		...
		<div class="meta_form_row">
			<label for="content"><?php _e('Notes', 'tsml')?></label>
	...
	add_meta_box('location', __('Location Information', 'tsml'), 'tsml_location_box', TSML_TYPE_MEETINGS, 'normal', 'low');
	...
	function tsml_location_box() {
		...
		<div class="meta_form_row typeahead">
			<label for="location"><?php _e('Location', 'tsml')?></label>
			<input type="text" name="location" id="location" value="<?php echo @$location->post_title?>">
		</div>
		<div class="meta_form_row">
			<label for="formatted_address"><?php _e('Address', 'tsml')?></label>
		...
		<div class="meta_form_row">
			<label for="region"><?php _e('Region', 'tsml')?></label>
		...
		<div class="meta_form_row">
			<label><?php _e('Map', 'tsml')?></label>
			<div id="map"></div>
		</div>
		<?php if (count($meetings) > 1) {?>
		<div class="meta_form_row">
			<label><?php _e('Meetings', 'tsml')?></label>
			<ol>
		...
		<?php }?>
		<div class="meta_form_row">
			<label><?php _e('Notes', 'tsml')?></label>
			<textarea name="location_notes" placeholder="<?php _e('eg. Around back, basement, ring buzzer', 'tsml')?>"><?php echo @$location->post_content?></textarea>
		</div>
		<?php
	}
	
	add_meta_box('group', __('Group Information', 'tsml') . ' <span>(' . __('Optional', 'tsml') . ')</span>', 'tsml_group_box', TSML_TYPE_MEETINGS, 'normal', 'low');
	...
	function tsml_group_box() {
		...
		?>
		<div class="meta_form_row typeahead">
			<label for="group"><?php _e('Group', 'tsml')?></label>
		...
		<div class="meta_form_row checkbox apply_group_to_location hidden">
			<label><input type="checkbox" name="apply_group_to_location"> <?php _e('Apply this group to all meetings at this location', 'tsml')?></label>
		</div>
		<?php if (count($meetings) > 1) {?>
		<div class="meta_form_row">
		...
		<div class="meta_form_row">
			<label><?php _e('Notes', 'tsml')?></label>
			<textarea name="group_notes" placeholder="<?php _e('eg. Group history, when the business meeting is, etc.', 'tsml')?>"><?php echo @$group->post_content?></textarea>
		</div>
		<div class="meta_form_row" style="clear:left;">
			<label><?php _e('Contacts', 'tsml')?></label>
			<div class="container">
				<?php for ($i = 1; $i <= GROUP_CONTACT_COUNT; $i++) {?>
				<div class="row">
					<div><input type="text" name="contact_<?php echo $i?>_name" placeholder="<?php _e('Name', 'tsml')?>" value="<?php echo @$group_custom['contact_' . $i . '_name'][0]?>"></div>
					<div><input type="text" name="contact_<?php echo $i?>_email" placeholder="<?php _e('Email', 'tsml')?>" value="<?php echo @$group_custom['contact_' . $i . '_email'][0]?>"></div>
					<div><input type="text" name="contact_<?php echo $i?>_phone" placeholder="<?php _e('Phone', 'tsml')?>" value="<?php echo @$group_custom['contact_' . $i . '_phone'][0]?>"></div>
				</div>
				<?php }?>
			</div>
		</div>
